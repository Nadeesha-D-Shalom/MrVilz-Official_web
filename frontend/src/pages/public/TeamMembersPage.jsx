...
function TeamMemberCard({ member }) {
  return (
    ...
    >
      ...
      <div className="p-5">
        <h2 className="font-display text-xl font-bold text-brand-ink">{member.name}</h2>
        <p className="mt-1 text-sm font-semibold text-brand-red">{member.position}</p>
      ...
      <div className="mx-auto max-w-6xl">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-base leading-relaxed text-brand-brown-lt">
            Select a member to read their full story, bio, and role at MrVilz.
          </p>
        </div>
        ...
        {!loading && !team.length ? (
          <p className="mt-12 rounded-2xl border border-brand-ink/10 bg-white px-6 py-8 text-center text-brand-brown-lt">
            Team profiles will appear here once they are published from the admin dashboard.
          </p>
        ) : null}
